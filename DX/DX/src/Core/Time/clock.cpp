...
namespace Core { namespace Time {
    ...
    //----------------------------------------------------------------------
    CallbackID Clock::setInterval( const std::function<void()>& func, F32 ms )
    {
        return _AttachCallback( func, ms, true );
    }

    //----------------------------------------------------------------------
    CallbackID Clock::setTimeout( const std::function<void()>& func, F32 ms )
    {
    ...
    //----------------------------------------------------------------------
    CallbackID Clock::_AttachCallback(const std::function<void()>& func, F32 ms, bool loop)
    {
        CallbackInfo info;
